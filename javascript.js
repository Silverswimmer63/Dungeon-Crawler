...
function _makeItem(){
   ...
   else if (item.type == Potion) { var retItem = new item.type(item.name, item.type, item.value, item.desc, item.icon, item.damage, item.level) }
...
function randomFoe(level) {
  ...
  Bowen HW
  randomItem(level)
  takes your level at the moment and checks to see what items it should give you. Generally speaking, you should get worse loot in comparison to your level and get
  ...
  1. Make multiple if statements that will function differently based on the the player's current level/status.
  2.
