  ...
  class Map{
    constructor(width,height){
      ...
      */
      ...
      /*placementCheck()
       *this checks weather the placement for the doors is completely valid or not
       */
      placementCheck(map,coords){
        var yes = 0;
        var no = 0;
        var cellyp = map["y" + (coords.y+1)]["x" + (coords.x)];
        var cellym = map["y" + (coords.y-1)]["x" + (coords.x)];
        var cellxp = map["y" + (coords.y)]["x" + (coords.x+1)];
        var cellxm = map["y" + (coords.y)]["x" + (coords.x-1)];
        var cells = {xp:cellxp,yp:cellyp,xm:cellxm,ym:cellym};
          if((cells.xp._type == "wall" && cells.xm._type == "wall")||(cells.yp._type == "wall" && cells.ym._type == "wall")){
            yes+=50;
          }
          if(no > yes || no == yes){
            var good = false;
        ...
        for(var i = 0; i < objArr.length; i++){
          var foundit = []
            for(var j = 0; j < objArr[i].length; j++){
              let cell = map["y" + (objArr[i][j].y+1)]["x" + (objArr[i][j].x+1)];
              if(cell.type !== "room"){
                cell._image = " "; // todo update type to set the image then have ranked inventy
                cell._type = "hall";
              }
                if(cell.type == "hall"){
                  var coor = {y:objArr[i][j].y+1, x:objArr[i][j].x+1,place:itter};
                  itter++;
                  foundit.push(coor);
                }
            }
              this._halls.push(foundit);
              itter = 0;
        }
      }
      ...
      /*getRelivents()
       *helper function
       *gets the absolute required coords for posible doors
       */
      getRelivents(){
        var testers = []
          for(var i = 0; i < this._halls.length;i++){
            for(var j = 0; j < this._halls[i].length;j++){
              if((j == 0)||(j == this._halls[i].length-1)){
                testers.push(this._halls[i][j]);
              }
            }
          }
        return testers;
      ...
      /*_placedoors()
       *needs map
       *@returns the doors and places them
       *uses helper functions getRelivents and placementCheck
       */
      _placedoors(map,percent = 0.9){
        var itter = 0;
        var rel = this.getRelivents();
          var percentNum = percent*100;
            for(var i = 0; i < rel.length; i++){
              var random = Utils.randMath(1,100);
                if(random <= percentNum){
                  let cell = map["y" + (rel[i].y)]["x" + (rel[i].x)];
                  var checker = this.placementCheck(map,rel[i]);
                  if(checker !== false){
                    cell.image = "x";
                  }
                }
      ...
      /*makeMonsters()
       *@param {integer} perRoom: the number per room there will be... kinda
       */
        ...
        var validRooms = [];
          ...
          while(mobItter < perRoom){
            for(var i = 0; i < validRooms.length; i++){
              ...
              var roompusher = {x:room.x,y:room.y};
              this._monsters.push(roompusher);
      ...
      /*setPlayer()
       *setplayer uses the map and the coords on the map to make the player set to a new location or just the start
       */
      ...
      /*setBeginningPos()
       *This function sets the beggining location for the player
       */
      _setBeginningPos(){
      ...
      /*changePosition
       *@param {string} direction tells witch way to move
       *@returns the "new" player moving AKA: the new position of the player 
       */
